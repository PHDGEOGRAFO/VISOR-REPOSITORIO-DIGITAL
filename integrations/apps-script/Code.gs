  ...
  for (let r = 1; r < values.length; r++) {
    ...
    if (normalizeEmail_(values[r][0]) === email) {
      ...
      sh.getRange(r + 1, 2, 1, 3).setValues([[
        'SI',
        now_(),
        'Autorizado desde solicitud del Visor Territorial'
      ]]);
...
function findPendingByToken_(token) {
  ...
  for (let r = 1; r < values.length; r++) {
    ...
    if (
      String(values[r][5] || '') === token &&
      String(values[r][4] || '').toUpperCase() === 'PENDIENTE'
    ) {
      return {
        sheet: sh,
        row: r + 1,
        email: normalizeEmail_(values[r][1]),
        coverage: String(values[r][2] || ''),
        format: String(values[r][3] || '')
      };
    }
  }
...
function requestAccess_(payload) {
  ...
  const coverage = String(
    payload.coverage || 'Cobertura territorial'
  );
  const format = String(
    payload.format || 'Archivo'
  );
  ...
  if (isAuthorized_(email)) {
    return {
      ok: true,
      authorized: true,
      message: 'El correo ya se encuentra autorizado.'
    };
  }

  const token = Utilities.getUuid();

  sheet_(SHEET_REQUESTS).appendRow([
    now_(),
    email,
    coverage,
    format,
  ...
  const subject =
    'Solicitud de acceso · Visor Territorial';
  ...
  return {
    ok: true,
    message:
      'Solicitud enviada. Recibirá un correo cuando su acceso sea autorizado.'
  };
}
...
function registerDownload_(payload) {
  ...
  const dimension = String(
    payload.dimension || 'SIN DIMENSION'
  );

  const sector = String(
    payload.sector || 'SIN SECTOR'
  );

  const anio = String(
    payload.anio || 'SIN AÑO'
  );

  let result = 'RECHAZADO';

  let message =
    'Correo no autorizado o clave incorrecta.';
...
function doPost(e) {
  try {
    ...
    if (payload.action === 'request') {
      return json_(
        requestAccess_(payload)
      );
    }

    if (payload.action === 'download') {
      return json_(
        registerDownload_(payload)
      );
    }
...
function doGet(e) {
  ...
  if (
    ...
    try {
      ...
      const result =
        action === 'request'
          ? requestAccess_(params)
          : registerDownload_(params);

      return callback
        ? jsonp_(callback, result)
        : json_(result);
  ...
  if (
    ...
    return HtmlService.createHtmlOutput(
      '<h3>Visor Territorial</h3>' +
      '<p>Servicio de autorización activo.</p>'
    );
  }


  const request =
    findPendingByToken_(token);
  ...
  if (!request) {
    ...
    return HtmlService.createHtmlOutput(
      '<h3>Visor Territorial</h3>' +
      '<p>La solicitud no existe, ya fue resuelta o el enlace no es válido.</p>'
    );
  }
  ...
  if (action === 'reject') {
    ...
    request.sheet
      .getRange(request.row, 5)
      .setValue('RECHAZADO');

    request.sheet
      .getRange(request.row, 7)
      .setValue(now_());
  ...
  MailApp.sendEmail(
    request.email,
    'Acceso autorizado al Visor Territorial',
    [
  ...
  return HtmlService.createHtmlOutput(
    '<h3>Visor Territorial</h3>' +
    '<p>Usuario autorizado correctamente. Se envió el correo de confirmación.</p>'
  );
}
